//! BridgedDeviceBasicInformation cluster (0x0039).
...
pub mod command_id {
    /// `KeepActive` (request).
    pub const KEEP_ACTIVE: u32 = 0x80;
}
...
#[must_use]
#[allow(clippy::expect_used, clippy::missing_panics_doc)] // Vec-backed TlvWriter is infallible.
pub fn encode_keep_active(stay_active_duration: u32, timeout_ms: u32) -> Vec<u8> {
    let mut buf = Vec::new();
    let mut w = TlvWriter::new(&mut buf);
    w.start_structure(Tag::Anonymous)
        .expect("infallible: vec writer");
    w.put_uint(Tag::Context(0), u64::from(stay_active_duration))
        .expect("infallible: vec writer");
    w.put_uint(Tag::Context(1), u64::from(timeout_ms))
        .expect("infallible: vec writer");
    w.end_container().expect("infallible: vec writer");
    buf
}
